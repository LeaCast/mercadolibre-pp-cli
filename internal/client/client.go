...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "mercadolibre-pp-cli", "http")
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export MERCADOLIBRE_ACCESS_TOKEN=<your-token> or mercadolibre-pp-cli auth set-token <token>")
}
