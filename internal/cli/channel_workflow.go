...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  mercadolibre-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  mercadolibre-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"countries", "sites"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  mercadolibre-pp-cli workflow status

  # Show status as JSON
  mercadolibre-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("mercadolibre-pp-cli")
			}
