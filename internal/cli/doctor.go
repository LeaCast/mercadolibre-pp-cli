...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mercadolibre-pp-cli doctor
  mercadolibre-pp-cli doctor --json
  mercadolibre-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export MERCADOLIBRE_ACCESS_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "MERCADOLIBRE_ACCESS_TOKEN")
				if os.Getenv("MERCADOLIBRE_ACCESS_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "MERCADOLIBRE_ACCESS_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("MERCADOLIBRE_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "MERCADOLIBRE_ACCESS_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "MERCADOLIBRE_ACCESS_TOKEN optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "mercadolibre-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("mercadolibre-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'mercadolibre-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'mercadolibre-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'mercadolibre-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'mercadolibre-pp-cli sync' to refresh."
	}
	return report
}
