...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "mercadolibre-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MERCADOLIBRE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MERCADOLIBRE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mercadolibre-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/mercadolibre-pp-cli/feedback.jsonl.
When ` + "`MERCADOLIBRE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MERCADOLIBRE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mercadolibre-pp-cli feedback list
  mercadolibre-pp-cli feedback list --limit 5
  mercadolibre-pp-cli feedback list --json`,
