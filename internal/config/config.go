...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("MERCADOLIBRE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "mercadolibre-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MERCADOLIBRE_ACCESS_TOKEN"); v != "" {
		cfg.AccessToken = v
		cfg.AuthSource = "env:MERCADOLIBRE_ACCESS_TOKEN"
	}
	...
	if v := os.Getenv("MERCADOLIBRE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AccessToken != "" {
		c.AuthSource = "env:MERCADOLIBRE_ACCESS_TOKEN"
		return applyAuthFormat("Bearer {token}", map[string]string{
			"access_token":              c.AccessToken,
			"MERCADOLIBRE_ACCESS_TOKEN": c.AccessToken,
			"token":                     c.AccessToken,
		})
	}
	if c.AccessToken != "" {
